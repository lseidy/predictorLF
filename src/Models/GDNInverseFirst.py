...
class GDN_NN(nn.Module):
    def __init__(self, name, params):
        ...
        flat_model = type_mode([  # 18, 64²
            nn.Sequential(
                Conv2d(1, n_filters, 3, stride=1, padding=1), GDN(n_filters),  # 10, 64²
                Conv2d(n_filters, n_filters, 3, stride=2, padding=1), GDN(n_filters),  # 10, 32²
            ),
            nn.Sequential(
                Conv2d(n_filters, (n_filters * 2), 3, stride=1, padding=1), GDN(n_filters*2),  # 10, 32²
                Conv2d((n_filters*2), (n_filters*2), 3, stride=2, padding=1), GDN(n_filters*2),  # 10, 16²
            ),
            nn.Sequential(
                Conv2d((n_filters*2), (n_filters*4), 3, stride=1, padding=1), GDN(n_filters*4),  # 10, 16²
                Conv2d((n_filters*4), (n_filters*4), 3, stride=2, padding=1), GDN(n_filters*4),  # 10, 8²
            ),
            nn.Sequential(
                Conv2d((n_filters*4), (n_filters*8), 3, stride=1, padding=1), GDN(n_filters*8),  # 10, 8²
                Conv2d((n_filters*8), (n_filters*8), 3, stride=2, padding=1), GDN(n_filters*8),  # 10, 4²
            ),
            nn.Sequential(
                Conv2d((n_filters*8), 512, 3, stride=1, padding=1), GDN(512),  # 10, 4
            ),
            ...
            nn.Sequential( 
                ...
                nn.Conv2d(512, n_filters*4, kernel_size=3, stride=1, padding=1), 
            ),
            nn.Sequential(
                GDN(n_filters*4, inverse=True),
                nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),  # 16
                nn.Conv2d(mul_fact*(n_filters*4), n_filters * 2, kernel_size=3, stride=1, padding=1)
            ),
            nn.Sequential(  
                GDN(n_filters*2, inverse=True),
                nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),  # 32
                nn.Conv2d(mul_fact * (n_filters * 2), n_filters, kernel_size=3, stride=1, padding=1),
            ),
            nn.Sequential(  
                GDN(n_filters, inverse=True),
