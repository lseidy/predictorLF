    ...
    def __init__(self, name, params):
        ...
        flat_model = UNetLike([  # 18, 64²
            ...
            nn.Sequential(  # 10, 4
                ...
                Conv2d(512, (n_filters*4), 3, stride=1, padding=1), nn.PReLU(),  # 1,  8
            ),
            nn.Sequential(  # 10, 510²
                nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False), #16
                Conv2d((n_filters*8), (n_filters*2), 3, stride=1, padding=1), nn.PReLU(),  # 1, 16
            ),
            nn.Sequential(  # 10, 510²
                nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),#32
                Conv2d((n_filters*4), (n_filters), 3, stride=1, padding=1), nn.PReLU(),  # 1, 32
            ),
            nn.Sequential(  # 10, 510²a
                # nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False),
                nn.ConvTranspose2d((n_filters*2), 1, 4, stride=2, padding=1), nn.PReLU(),  # 1, 64
            ),
            nn.Sequential(  # 10, 510²
